...
class EmotionAwareMultiHeadedAttention(nn.Module):
    def __init__(self, head_count, model_dim, num_emotions, dropout=0.1):
        assert model_dim % head_count == 0
        self.dim_per_head = model_dim // head_count
        self.model_dim = model_dim

        super(EmotionAwareMultiHeadedAttention, self).__init__()
        ...
        self.emotion_embeddings = nn.Parameter(
            torch.randn(num_emotions, model_dim)
        )
        self.emotion_projection = nn.Linear(model_dim, head_count * self.dim_per_head)
        
        self.softmax = nn.Softmax(dim=-1)
        self.dropout = nn.Dropout(dropout)
        self.output_layer = nn.Linear(model_dim, model_dim)

    def forward(self, key, value, query, emotion_labels=None, mask=None):
        ...
        if emotion_labels is not None:
            # Get emotion embeddings for the batch
            batch_emotion_embeddings = self.emotion_embeddings[emotion_labels]
            emotion_query = shape(self.emotion_projection(batch_emotion_embeddings))
            
            # Combine with original query
            query = query + emotion_query.unsqueeze(2)
...
class EmotionAwareTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, heads, d_ff, num_emotions, dropout):
        super(EmotionAwareTransformerEncoderLayer, self).__init__()
        self.self_attn = EmotionAwareMultiHeadedAttention(
            heads, d_model, num_emotions, dropout=dropout)
        self.feed_forward = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.layer_norm = nn.LayerNorm(d_model, eps=1e-6)
        self.dropout = nn.Dropout(dropout)

    def forward(self, iter, inputs_a, inputs_b, emotion_labels=None, mask=None):
        if inputs_a.equal(inputs_b):
            ...
            context = self.self_attn(inputs_b, inputs_b, inputs_b, 
                                   emotion_labels=emotion_labels, mask=mask)
        else:
            if (iter != 0):
                inputs_b = self.layer_norm(inputs_b)
...
class EmotionAwareTransformerEncoder(nn.Module):
    def __init__(self, d_model, d_ff, heads, layers, num_emotions, dropout=0.1):
        super(EmotionAwareTransformerEncoder, self).__init__()
        self.d_model = d_model
        self.layers = layers
        self.pos_emb = PositionalEncoding(d_model)
        self.transformer_inter = nn.ModuleList(
            [EmotionAwareTransformerEncoderLayer(d_model, heads, d_ff, num_emotions, dropout)
             for _ in range(layers)])
        self.dropout = nn.Dropout(dropout)

    def forward(self, x_a, x_b, mask, speaker_emb, emotion_labels=None):
        if x_a.equal(x_b):
            x_b = self.pos_emb(x_b, speaker_emb)
            x_b = self.dropout(x_b)
            for i in range(self.layers):
                x_b = self.transformer_inter[i](i, x_b, x_b, emotion_labels, mask.eq(0))
        else:
            ...
            x_b = self.dropout(x_b)
            for i in range(self.layers):
                x_b = self.transformer_inter[i](i, x_a, x_b, emotion_labels, mask.eq(0))
        return x_b
...
class EmotionAwareTransformerModel(nn.Module):
    def __init__(self, dataset, temp, D_text, D_visual, D_audio, n_head,
        ...
        self.t_t = EmotionAwareTransformerEncoder(
            d_model=hidden_dim, d_ff=hidden_dim, heads=n_head, 
            layers=1, num_emotions=n_classes, dropout=dropout)
        self.a_t = EmotionAwareTransformerEncoder(
            d_model=hidden_dim, d_ff=hidden_dim, heads=n_head, 
            layers=1, num_emotions=n_classes, dropout=dropout)
        self.v_t = EmotionAwareTransformerEncoder(
            d_model=hidden_dim, d_ff=hidden_dim, heads=n_head, 
            layers=1, num_emotions=n_classes, dropout=dropout)

        self.a_a = EmotionAwareTransformerEncoder(
            d_model=hidden_dim, d_ff=hidden_dim, heads=n_head, 
            layers=1, num_emotions=n_classes, dropout=dropout)
        self.t_a = EmotionAwareTransformerEncoder(
            d_model=hidden_dim, d_ff=hidden_dim, heads=n_head, 
            layers=1, num_emotions=n_classes, dropout=dropout)
        self.v_a = EmotionAwareTransformerEncoder(
            d_model=hidden_dim, d_ff=hidden_dim, heads=n_head, 
            layers=1, num_emotions=n_classes, dropout=dropout)

        self.v_v = EmotionAwareTransformerEncoder(
            d_model=hidden_dim, d_ff=hidden_dim, heads=n_head, 
            layers=1, num_emotions=n_classes, dropout=dropout)
        self.t_v = EmotionAwareTransformerEncoder(
            d_model=hidden_dim, d_ff=hidden_dim, heads=n_head, 
            layers=1, num_emotions=n_classes, dropout=dropout)
        self.a_v = EmotionAwareTransformerEncoder(
            d_model=hidden_dim, d_ff=hidden_dim, heads=n_head, 
            layers=1, num_emotions=n_classes, dropout=dropout)
    ...
    def forward(self, textf, visuf, acouf, u_mask, qmask, dia_len, emotion_labels=None):
        ...
        # Intra- and Inter-modal Transformers
        t_t_transformer_out = self.t_t(textf, textf, u_mask, spk_embeddings, emotion_labels)
        a_t_transformer_out = self.a_t(acouf, textf, u_mask, spk_embeddings, emotion_labels)
        v_t_transformer_out = self.v_t(visuf, textf, u_mask, spk_embeddings, emotion_labels)

        a_a_transformer_out = self.a_a(acouf, acouf, u_mask, spk_embeddings, emotion_labels)
        t_a_transformer_out = self.t_a(textf, acouf, u_mask, spk_embeddings, emotion_labels)
        v_a_transformer_out = self.v_a(visuf, acouf, u_mask, spk_embeddings, emotion_labels)

        v_v_transformer_out = self.v_v(visuf, visuf, u_mask, spk_embeddings, emotion_labels)
        t_v_transformer_out = self.t_v(textf, visuf, u_mask, spk_embeddings, emotion_labels)
        a_v_transformer_out = self.a_v(acouf, visuf, u_mask, spk_embeddings, emotion_labels)
